...
import AddAccommodation from "../add/AddPenginapan";
...
// Format float or string price to Indonesian Rupiah
const formatToRupiah = (value: string | number): string => {
    ...
    // Convert format: 1.234,56 => 1.234,56 (already what we want in id-ID)
    return `Rp. ${formatted}`;
};
        ...
        <div className="max-w-6xl px-6 py-6 mx-auto">
            ...
            {activeTab === "list" && (
                <div className="p-6 mt-4 bg-white border border-gray-200 rounded-md shadow-md">
                    <h1 className="pb-2 mb-4 text-2xl font-bold text-center text-gray-800">Daftar Penginapan</h1>
                    <div className="overflow-x-auto">
                        <table className="min-w-full text-sm border border-gray-200 table-auto">
                            <thead className="text-left text-gray-700 bg-gray-100">
                                <tr>
                                    <th className="w-1/4 px-4 py-2 border">Nama Penginapan</th>
                                    <th className="w-1/5 px-4 py-2 border">Harga per Malam</th>
                                    <th className="w-1/3 px-4 py-2 border">Check-in / Check-out</th>
                                    <th className="w-1/6 px-4 py-2 text-center border">Aksi</th>
                                </tr>
                            </thead>
                            <tbody>
                                {accommodations.length > 0 ? (
                                    accommodations.map((item) => (
                                        <tr key={item.id} className="transition hover:bg-gray-50">
                                            ...
                                            <td className="px-4 py-2 text-gray-700 border">{formatToRupiah(item.price)}</td>
