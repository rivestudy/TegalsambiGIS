            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {data.map((item) => (
                    <Link to={`/${itemUrlPrefix}/${item.id}`} key={item.id}>
                        <div className={`${cardColor} shadow-md rounded-2xl overflow-hidden flex flex-col h-[250px] transition transform duration-300 hover:shadow-lg hover:-translate-y-2 cursor-pointer`}>
